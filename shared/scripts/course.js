...
function initCourse(courseName, badges) {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const renderCourseView = (semesterIdx) => {
            ...
            semester.courses.forEach((course, courseIdx) => {
                ...
                card.innerHTML = `
                    ...
                    <div class="checklist">
                        <label><input type="checkbox" data-type="main" ${courseProgress[courseId].main ? 'checked' : ''}> Aula Principal</label>
                        <label><input type="checkbox" data-type="reading" ${courseProgress[courseId].reading ? 'checked' : ''}> Leitura Recomendada</label>
                    </div>
                ...
                card.querySelector('.checklist').addEventListener('change', (e) => {
                    courseProgress[courseId][e.target.dataset.type] = e.target.checked;
                    saveProgress();
                    renderDashboard();
                });
        ...
        const startTimer = () => {
            ...
            timerInterval = setInterval(() => {
                ...
                if (timerSeconds <= 0) {
                    ...
                    storage.addPomodoroSession(courseName, 25);
                ...
                semester.courses.forEach(course => {
                    if (course.prerequisites && course.prerequisites !== "N/A") {
                        ...
                        prereqs.forEach(prereq => {
                            const foundPrereq = Array.from(allCourseTitles).find(title => title.toLowerCase().includes(prereq.toLowerCase()));
                            if (foundPrereq) {
        ...
        const showWelcomeMessage = () => {
            const courseDisplayName = courseName.replace('-', ' ');
            const welcomeText = `Olá! 👋 Sou seu tutor de IA. Como posso ajudar com seus estudos de ${courseDisplayName} hoje?`;
        ...
        const handleSendMessage = async () => {
            ...
            try {
                let context = `O aluno está estudando ${courseName}.`;
                if (currentSemesterIndex !== -1) {
                    ...
                    context += ` Ele está visualizando a ${semester.semester}ª Etapa, que inclui: ${courseTitles}.`;
                }
        ...
        toggleViewBtn.addEventListener('click', () => {
            if (currentView === 'dashboard') showDependencyView();
            else showDashboard();
        });
